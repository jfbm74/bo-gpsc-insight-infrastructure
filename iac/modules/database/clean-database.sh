...
# ==============================================================================
# BLUE OWL GPS REPORTING - DATABASE MODULE CLEANUP
...
show_usage() {
    ...
    echo ""
    echo "Clean up Blue Owl GPS SQL Database resources (PRESERVES Resource Group & other infrastructure)"
    echo ""
    echo "Options:"
    ...
    echo ""
    echo "Examples:"
    echo "  # Delete SQL Database resources (keeps Resource Group & other infrastructure)"
    echo "  $0 -e dev"
    echo ""
    ...
    echo "  • SQL Server"
    echo "  • SQL Database"
    echo "  • Related security policies"
    echo "  • Database data (IRREVERSIBLE)"
}
...
║                                                                              ║
║  🧹 CLEAN SQL DATABASE RESOURCES ONLY                                       ║
...
echo "  • Security Policies:  All related policies"
echo "  • Database Data:      All data (PERMANENT DELETION)"
echo ""
...
if az sql server show --name "$SQL_SERVER_NAME" --resource-group "$RESOURCE_GROUP_NAME" &> /dev/null; then
    ...
    # Check for database
    if az sql db show --server "$SQL_SERVER_NAME" --name "$SQL_DATABASE_NAME" --resource-group "$RESOURCE_GROUP_NAME" &> /dev/null; then
...
if [[ "$SQL_SERVER_EXISTS" == false && "$SQL_DATABASE_EXISTS" == false ]]; then
    echo "  ℹ️  No SQL resources found to delete"
fi
...
SQL_COUNT=0
[[ "$SQL_SERVER_EXISTS" == true ]] && ((SQL_COUNT++))
[[ "$SQL_DATABASE_EXISTS" == true ]] && ((SQL_COUNT++))

echo ""
print_info "📊 Summary:"
echo "  • Total resources in RG: $TOTAL_RESOURCES"
echo "  • SQL resources: $SQL_COUNT (will be deleted)"
echo "  • Other resources: $((TOTAL_RESOURCES - SQL_COUNT)) (will be preserved)"

# Check if any SQL resources exist
if [[ "$SQL_SERVER_EXISTS" == false && "$SQL_DATABASE_EXISTS" == false ]]; then
    print_warning "No SQL Database resources found in resource group '$RESOURCE_GROUP_NAME'"
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    if [[ "$SQL_DATABASE_EXISTS" == true ]]; then
        echo "    • SQL Database: $SQL_DATABASE_NAME"
        echo "      - All database data (PERMANENT)"
        echo "      - All tables and stored procedures"
        echo "      - All user accounts and permissions"
    fi
    if [[ "$SQL_SERVER_EXISTS" == true ]]; then
        echo "    • SQL Server: $SQL_SERVER_NAME"
    ...
    echo "    • All other non-SQL resources"
...
if [[ "$SQL_DATABASE_EXISTS" == true ]]; then
    echo "    • SQL Database: $SQL_DATABASE_NAME"
    echo "    • ALL database data and tables"
    echo "    • ALL stored procedures and functions"
    echo "    • ALL user accounts and permissions"
    echo "    • ALL transaction logs"
fi
if [[ "$SQL_SERVER_EXISTS" == true ]]; then
    echo "    • SQL Server: $SQL_SERVER_NAME"
    echo "    • Server configuration and policies"
    echo "    • Audit and security settings"
fi
...
if [[ "$SKIP_CONFIRMATION" != true ]]; then
    print_error "📋 CRITICAL: This script ONLY deletes SQL Database resources."
    ...
    if [[ "$SQL_SERVER_EXISTS" == true || "$SQL_DATABASE_EXISTS" == true ]]; then
        print_error "🚨 Type 'DELETE-ALL-DATABASE-DATA' to confirm PERMANENT deletion of all SQL data:"
...
# Step 2: Delete SQL Server (after database is deleted)
if [[ "$SQL_SERVER_EXISTS" == true ]]; then
...
SQL_DATABASE_STILL_EXISTS=false
...
if [[ "$SQL_SERVER_STILL_EXISTS" == false ]] && az sql db show --server "$SQL_SERVER_NAME" --name "$SQL_DATABASE_NAME" --resource-group "$RESOURCE_GROUP_NAME" &> /dev/null 2>&1; then
    SQL_DATABASE_STILL_EXISTS=true
    print_warning "SQL Database still exists: $SQL_DATABASE_NAME"
fi
...
if [[ "$SQL_SERVER_STILL_EXISTS" == false && "$SQL_DATABASE_STILL_EXISTS" == false ]]; then
    print_success "✅ SQL resources successfully deleted"
    print_success "✅ All database data permanently removed"
else
    print_warning "⚠️  Some SQL resources may still exist (deletion in progress)"
fi
...
echo ""
echo "🧹 SQL Database resources have been cleaned up successfully"
echo "📁 Resource Group '$RESOURCE_GROUP_NAME' has been PRESERVED"
echo ""

echo "✅ Successfully DELETED (SQL resources only):"
if [[ "$SQL_DATABASE_EXISTS" == true ]]; then
    echo "  🗑️  SQL Database: $SQL_DATABASE_NAME (all data deleted)"
fi
if [[ "$SQL_SERVER_EXISTS" == true ]]; then
    echo "  🗑️  SQL Server: $SQL_SERVER_NAME"
fi
...
echo "📋 Verify cleanup:"
echo "  # Confirm SQL resources are gone"
...
echo "🔄 To redeploy SQL Database:"
echo "  ./deploy-database.sh -e $ENVIRONMENT -g $RESOURCE_GROUP_NAME"
echo ""

echo "💡 Next steps:"
echo "  • SQL Database infrastructure has been removed"
echo "  • All other infrastructure remains intact"
echo "  • Backend App Service will need database configuration updated"
echo "  • Ready for fresh SQL Database deployment"
